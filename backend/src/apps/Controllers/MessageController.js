...
class MessageController{
    async sendMessage(req, res) {
        ...
        if (user_destination === user_send) {
            return res.status(400).json({
                message: "Você não pode enviar mensagem para vocês mesmo!"
            })
        }
        ...
        const receiver = crypto.createHash('md5').update(`${user_destination}`)
            .digest('hex')
    ...
    async listAllMessages(req, res){
        ...
        const messages = [...messagesReceiver, ...messageSend]

        if(messages.length === 0){
            return res.status(404).json({
                message:
                "Sem mensagem para carregar"
            })
        }
        const formattedData = [];
